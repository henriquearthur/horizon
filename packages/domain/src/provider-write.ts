...
export class ProviderWriteError extends Error {
  ...
  constructor(message: string, status: number) {
    ...
    this.status = status
    this.code = status === 401 ? 'unauthorized' : status === 403 ? 'forbidden' : status >= 500 ? 'unavailable' : 'invalid-response'
  }
}

const asUser = (value: any): ProviderUser | undefined =>
  typeof value?.id === 'number' && typeof value?.username === 'string'
    ? { id: value.id, username: value.username, name: typeof value.name === 'string' ? value.name : value.username, ...(typeof value.avatar_url === 'string' ? { avatarUrl: value.avatar_url } : {}) }
    : undefined

const asIssue = (value: any, projectId: number): ProviderIssue => {
  if (!(typeof value?.id === 'number' && typeof value?.iid === 'number' && typeof value?.title === 'string' && (value?.state === 'opened' || value?.state === 'closed') && typeof value?.web_url === 'string'))
    throw new ProviderWriteError('Resposta inválida do GitLab.', 502)
  const author = asUser(value.author)
  return {
    id: value.id, iid: value.iid, projectId, title: value.title,
    ...(typeof value.description === 'string' ? { description: value.description } : {}),
    state: value.state, webUrl: value.web_url, ...(author ? { author } : {}),
    assignees: Array.isArray(value.assignees) ? value.assignees.flatMap((item: unknown) => { const mapped = asUser(item); return mapped ? [mapped] : [] }) : [],
    labels: Array.isArray(value.labels) ? value.labels.filter((item: unknown): item is string => typeof item === 'string') : [],
  }
}

const asComment = (value: any): ProviderComment => {
  if (!(typeof value?.id === 'number' && typeof value?.body === 'string' && typeof value?.created_at === 'string'))
    throw new ProviderWriteError('Resposta inválida do GitLab.', 502)
  const author = asUser(value.author)
  return { id: value.id, body: value.body, createdAt: value.created_at, system: value.system === true, ...(author ? { author } : {}) }
}
...
export class GitLabWriteProvider implements ProviderWriteContract {
  ...
  createIssue(input: CreateIssueInput) { const { projectId, ...fields } = input; return this.#request(`projects/${projectId}/issues`, { method: 'POST', body: JSON.stringify(this.#fields(fields)) }).then(value => asIssue(value, projectId)) }
...
export const createGitLabWriteProvider = (connection: ProviderConnection, fetcher?: typeof fetch): ProviderWriteContract => new GitLabWriteProvider(connection, fetcher)
