...
export class GitLabReadProvider implements ProviderReadContract {
  constructor(private readonly connection: { url: string; token: string }, private readonly fetcher: typeof fetch = fetch, private readonly options: { maxRetries?: number; retryDelayMs?: number } = {}) {}
  private async request(path: string, pageNo = 1): Promise<{ value: any; next: string | null }> {
    ...
    for (let attempt = 0; ; attempt++) {
      try { response = await this.fetcher(url, { headers: { 'PRIVATE-TOKEN': this.connection.token, Accept: 'application/json' } }) }
      catch (error) { if (attempt >= retries) throw new Error('Não foi possível conectar ao GitLab.'); await this.pause(attempt); continue }
  ...
  private transient(status: number) { return status === 408 || status === 429 || status >= 500 }
  private pause(attempt: number) { const delay = this.options.retryDelayMs ?? 0; return delay > 0 ? new Promise<void>(resolve => setTimeout(resolve, delay * 2 ** attempt)) : Promise.resolve() }
