...
const issue = (overrides = {}) => ({
  ...
  state: 'opened',
  web_url: 'https://gitlab/team/app/-/issues/3',
  labels: ['bug'],
  assignees: [],
  ...overrides,
})
...
describe('GitLabWriteProvider', () => {
  it('creates and edits issues using GitLab fields', async () => {
    const fetcher = vi
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify(issue({ title: 'Corrigida' }))))
    const provider = new GitLabWriteProvider(
    ...
    expect(fetcher.mock.calls[1]?.[1]).toMatchObject({
      method: 'PUT',
      body: JSON.stringify({ title: 'Corrigida', assignee_ids: [] }),
    })
  })
  ...
  it('retries safe PUT mutations but never retries POST', async () => {
    ...
    const fetcher = vi
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify(issue)))
    await expect(
      new GitLabWriteProvider({ url: 'https://gitlab.example', token: 'x' }, fetcher).updateIssue(
        1,
        3,
        { title: 'x' },
      ),
    ...
    await expect(
      new GitLabWriteProvider({ url: 'https://gitlab.example', token: 'x' }, post).createComment(
        1,
        3,
        'x',
      ),
  ...
  it('reads and publishes comments and sends close/reopen events', async () => {
    const note = {
      ...
      system: false,
      author: { id: 2, username: 'ana', name: 'Ana' },
    }
    const fetcher = vi
      .fn<typeof fetch>()
      .mockResolvedValueOnce(new Response(JSON.stringify([note])))
      .mockResolvedValueOnce(new Response(JSON.stringify(note)))
      .mockResolvedValueOnce(new Response(JSON.stringify(issue({ state: 'closed' }))))
      .mockResolvedValueOnce(new Response(JSON.stringify(issue())))
    const provider = new GitLabWriteProvider(
      { url: 'https://gitlab.example', token: 'secret' },
      fetcher,
    ...
    await provider.setIssueState(7, 3, 'closed')
    await provider.setIssueState(7, 3, 'opened')
  ...
  it('exposes a specific permission error without hiding the action', async () => {
    ...
    await expect(provider.createComment(7, 3, 'Oi')).rejects.toEqual(
      expect.objectContaining<Partial<ProviderWriteError>>({
        code: 'forbidden',
        status: 403,
